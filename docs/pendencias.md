...
| ~~**Régua de preço**~~ **DECIDIDA, desfeita e redecidida em 10/09/2026** — hoje é LINEAR: R$ 5,90 por criança no mensal, R$ 2,90 no anual, com mínimo e taxa marginal acima de 40. Código testado em `dominio/associacao/planos.js`. | [negocio.md](negocio.md) 1 | ~~Faixa de R$ 69 / 149 / 229~~ | A faixa foi ratificada em 06/09 e durou quatro dias: no limite de uma para a outra, ganhar UMA criança subia a conta em R$ 40 — 6,8 vezes a taxa por criança. O argumento de que "faixa encurta a conversa" era verdadeiro e não pagava esse preço |
| ~~**Carência da indicação**~~ **DECIDIDA em 06/09/2026** — só conta no 1º mês PAGO do indicado, mais 12 meses de duração fixa. Ver [decisão 22](decisoes.md). | [negocio.md](negocio.md) 3 | **Só conta no 1º mês PAGO do indicado** | Sem isso, cinco cadastros de fachada zeram a conta de quem indicou antes de existir um real de receita |
...
| **De onde vem o CPF/CNPJ do motorista** | [plano-fases.md](plano-fases.md) gateway | **Pedir no cadastro do motorista, não no orçamento.** O gateway não cria cliente sem documento, e o app não coleta esse campo em lugar nenhum — nem o cadastro, nem `montarContrato`. Hoje ele entra pela mão do dono e fica em `taxaParceiros/{uid}`, que só o dono lê. Isso funciona e não escala: é o dono digitando documento de terceiro a cada associado novo. ⚠️ Se for para `users`, o motorista passa a ler o próprio CPF — o que é justo — mas o campo precisa entrar na lista de proibidos das rules, senão ele edita o documento que está na cobrança |
