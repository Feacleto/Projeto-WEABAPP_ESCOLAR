...
| [`descontos.md`](descontos.md) | **pessoa e agente** | Antes de mexer em qualquer desconto: escada de fechamento, piso de fatura, indicação, condição de fundador. Decidido em 07/09/2026, e **supera partes das seções 5 e 7 do `negocio.md`** (marcadas no lugar). A Parte 1 é a política e fica; a **Parte 2 é a lista de mudanças no código e na landing, e deve ser apagada quando estiver feita**. O critério que governa tudo é o **teste da fila do portão** — se um desconto não sobrevive a ser dito em voz alta entre dois motoristas, ele não existe. |
